...
class Auxin_Welcome extends Auxin_Welcome_Base {
                    ...
                    foreach ( $tempList['templates'] as $key => $args ) {
                        ...
                        echo '<div class="aux-grid-item-inner">';
                            echo '<div class="aux-grid-template-media">';
                                echo '<img class="template_thumbnail" src='.$args['thumbnail'].'>';
                        if( ! $is_demo_allowed ) {
    ...
    public function get_demo_list( $type = 'demos', $url = 'http://api.phlox.pro/demos/all', $sanitize_key = 'auxin_available_demos' ) {
        ...
        if( $type === 'templates' ){
            ...
            $sanitize_key = 'auxin_available_templates';
        }
    ...
    public function ajax_uninstall(){
        ...
        // Remove Menus
        $menus = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}options WHERE option_name LIKE '%_auxin_demo_menu_old_id_%'", OBJECT );
